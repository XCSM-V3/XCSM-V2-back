# Author: Dilane PAFE
# Fichier: xcsm/tasks.py - Celery Tasks (Intégration de l'IA Sémantique)
...
2. extract_text_from_document_task: Extraction de texte + Enrichissement IA (MODULE 2)
...
# ==============================================================================
# TÂCHE 2: EXTRACTION DE TEXTE & ENRICHISSEMENT IA
# ==============================================================================

@shared_task(bind=True, max_retries=3)
def extract_text_from_document_task(self, fichier_id):
    """
    Extrait le texte, construit le JSON brut, et fait intervenir le Chef d'Orchestre IA.
    """
    try:
        ...
        with ProcessingLogger(fichier, 'EXTRACTION_TEXT', self.request.id) as extract_log:
            ...
            # 1. Extraction mécanique (Zéro perte)
            if ext == 'docx':
            ...
            if not json_structure or not json_structure.get("sections"):
                ...
                json_structure = {
                    ...
                    "sections": [{"type": "h1", "level": 1, "content": "Document", "children": []}]
                }
            
            # ------------------------------------------------------------------
            # 2. MODULE 2 : INTERVENTION DU CHEF D'ORCHESTRE SÉMANTIQUE
            # ------------------------------------------------------------------
            logger.info(f"🧠 [Extract Task {self.request.id}] Appel au Chef d'Orchestre Sémantique pour enrichissement...")
            json_structure = apply_semantic_orchestrator(json_structure)
            
            sections_count = len(json_structure.get('sections', []))
            extract_log.set_result({'sections_count': sections_count, 'ia_enriched': "metadata_ia" in json_structure})
            
            logger.info(f"✅ [Extract Task {self.request.id}] Extraction & IA réussies: {sections_count} sections")
...
def generate_granules_task(self, fichier_id, json_structure):
    ...
    try:
        ...
        with ProcessingLogger(fichier, 'GRANULATION', self.request.id) as granule_log:
            ...
            # Sauvegarde de la structure enrichie (metadata_ia incluses)
            mongo_db['fichiers_uploades'].update_one(
                {"fichier_source_id": str(fichier.id)},
                {
                    "$set": {
                        ...
                        "version": "3.0-ML-ASYNC" # Nouvelle version IA
                    }
                },
                upsert=True
            )
            ...
            granule_log.set_result({
                ...
                'cours_code': cours.matiere.code if hasattr(cours, 'matiere') and cours.matiere else "N/A",
                'granules_count': granules_count
            })
            
            logger.info(f"✅ [Granule Task {self.request.id}] Génération réussie: {granules_count} granules")
